...
//				System.out.println("收到的消息是："+msg);	
				if(msg.startsWith("<#LOGIN#>")){				//消息为登录
				...
				}else if(msg.startsWith("<#GET_MY_ALBUM_LIST#>")){			//消息为获取相册列表
					msg = msg.substring(21);			//提取内容
					...
					if(size == 0){//如果用户无相册
						...
						StringBuilder sb = new StringBuilder();
							for(String [] sa:albumList){
								...
								sb.append(sa[3]);
								sb.append("$");
								Blob blob = DBUtil.getAlbumCover(sa[0]);	//获得头像Blob数据
								byte [] buf = blob.getBytes(1l, (int)blob.length());	//获取字节数组
					...
					int size = din.readInt();			//读取封面图片的大小
					byte [] buf = new byte[size];		//创建字节数组
					for(int i=0;i<size;i++){
						buf[i] = din.readByte();
					}
					
					int result = DBUtil.createAlbum(buf,sa[0], sa[1]);		//创建新相册
					if(result == 1){		//创建成功
					...
					int result = DBUtil.insertPhotoFromAndroid(buf, sa[0], sa[1],lat ,lng,sa[2]);	//插入图片
					if(result == 1){
...
//						}
				...
				else if(msg.startsWith("<#GET_USER_HEAD#>")){
//					System.out.println(msg);
					msg = msg.substring(17);
//					System.out.println(msg);
					Blob blob = DBUtil.getHeadBlob(msg);	//获得头像Blob数据
					byte [] buf = blob.getBytes(1l, (int)blob.length());	//获取字节数组
					dout.writeInt(buf.length);
					dout.write(buf);		//将字节数组发出
				//	dout.flush();
